...
import { ArrowLeft, ArrowUp, Pencil, Trash2, X, Plus, Paperclip, Loader2 } from "lucide-react";
...
export default function SkillsSettingsPage() {
  ...
  const rosterPercent = totalCount ? Math.round((enabledCount / totalCount) * 100) : 0;

  const body = (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-[22px] font-semibold tracking-tight">Skills</h2>
          <p className="text-[12.5px] text-muted-foreground mt-1">
            Tune the experts Megsy can switch into inside chat.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <CleanButton variant="primary" size="sm" onClick={() => navigate("/settings/skills/new")}>
      ...
      <CleanCard title="Active roster" description="Enabled skills are available to Megsy in chat.">
        <div className="flex items-baseline gap-2">
          <span className="text-[40px] font-semibold tracking-tight leading-none">
            {enabledCount}
          </span>
          <span className="text-[14px] text-muted-foreground font-medium">/ {totalCount}</span>
        </div>
          ...
          {[
            ...
            return (
              <button
                ...
                onClick={() => setTab(t.id)}
                className={cn(
                  "flex items-center gap-2 px-4 h-9 text-[13px] font-medium rounded-lg transition-all duration-200",
                  active
              ...
              >
                ...
                <span
                  className={cn(
                    "min-w-[20px] h-[18px] inline-flex items-center justify-center text-[10.5px] font-semibold px-1.5 rounded-full transition-colors",
                    active
          ...
          <div className="rounded-xl border border-dashed border-border/60 py-14 text-center">
            ...
            <div className="mt-5 flex flex-col sm:flex-row items-center justify-center gap-2">
              ...
              <CleanButton
                ...
                disabled={importing}
              >
                <Paperclip className="w-3.5 h-3.5" /> Import .zip
              </CleanButton>
              <CleanButton variant="ghost" size="sm" onClick={() => setTab("library")}>
                Browse library
              </CleanButton>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {SUGGESTIONS.slice(0, 4).map((s) => (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-border/60 bg-card text-[12.5px] text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
            >
    ...
    <div className="relative min-h-[100dvh] overflow-y-auto bg-background text-foreground pb-16">
      <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-xl border-b border-border/60">
        <div className="max-w-lg mx-auto px-4 flex items-center justify-between py-3 safe-top">
          ...
          <h1 className="text-[17px] font-semibold tracking-tight">Skills</h1>
          <div className="w-10" />
        </div>
      </header>
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          />
          <p className="text-[14px] font-semibold text-foreground truncate">{skill.name}</p>
        </div>
        {skill.description && (
          <p className="text-[12.5px] text-muted-foreground line-clamp-2 mt-0.5 leading-snug">
            {skill.description}
        ...
        {skill.triggers && skill.triggers.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {skill.triggers.slice(0, 4).map((t, i) => (
              <span
                key={i}
                className="inline-flex items-center h-5 px-1.5 rounded bg-secondary text-[10.5px] font-medium text-muted-foreground"
              >
                /{t}
              </span>
            ))}
            {skill.triggers.length > 4 && (
              <span className="inline-flex items-center h-5 px-1.5 rounded bg-secondary text-[10.5px] font-medium text-muted-foreground">
                +{skill.triggers.length - 4}
              </span>
            )}
          </div>
      ...
      <div className="flex items-center gap-1">
        <button
          onClick={onEdit}
          className="grid h-8 w-8 place-items-center rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary/60 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all"
          aria-label="Edit"
        >
        ...
        <button
          onClick={onDelete}
          className="grid h-8 w-8 place-items-center rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all"
          aria-label="Delete"
        >
