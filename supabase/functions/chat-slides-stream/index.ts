...
import { createJob, runInBackground, JobWriter, loadJobForResume } from "../_shared/jobs.ts";
...
serve(async (req) => {
  ...
  if (isInternalCaller(req) && body?.action === "resume" && body?.jobId) {
    ...
    if (job.kind !== "slides") {
      return new Response(JSON.stringify({ error: "wrong_runner" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    const inp = (job.input ?? {}) as Record<string, any>;
    authUser = { id: job.user_id };
    reuseJobId = job.id;
    body = {
      ...inp,
      background: true,
